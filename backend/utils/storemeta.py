...
def store_meta(file_location, file_name, file_size, file_type, user_gmail):
    ...
    with conn.cursor() as curr:
        ...
        curr.execute(
            "CREATE TABLE IF NOT EXISTS files (file_id SERIAL PRIMARY KEY, file_location VARCHAR(255), file_name VARCHAR(255) UNIQUE, file_size INT, file_type VARCHAR(255), user_gmail VARCHAR(255))"
        )
